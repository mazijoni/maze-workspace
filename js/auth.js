...
/* ── Cross-domain account sync ── */

function _wirePortfolioLinks() {
    let accountsParam;
    try { accountsParam = btoa(JSON.stringify(_getSavedAccounts())); } catch { return; }
    document.querySelectorAll('a[href^="https://maze-development.com"]').forEach(link => {
        link.addEventListener('click', e => {
            e.preventDefault();
            const url = new URL(link.href);
            url.searchParams.set('accounts', accountsParam);
